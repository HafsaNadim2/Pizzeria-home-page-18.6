...
from django.urls import path # have to use path instead of url or will not work in new versions of Django # the same concept, this imports the url function, which is needed when mapping URLs to views
...
urlpatterns = [ # The variable urlpatterns in this module is a list of individual pages that can be requested from the pizzas app
    # Home page
    path('', views.index, name='index'),  # need to use path bc of new version of Django but same concept if it was url # In its entirety, this expression tells Python to look for a URL with nothing between the beginning and end of the URL
